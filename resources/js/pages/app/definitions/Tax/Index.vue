...
const deletingTax = ref<Tax | null>(null);
...
function openDeleteDialog(tax: Tax) {
...
function confirmDelete() {
    ...
    form.delete(destroy.url(deletingTax.value.id), {
        onSuccess: () => {
        ...
        <DefinitionsLayout>
                        ...
                        <Table>
                            <TableHeader>
                                <TableRow>
                                    ...
                                    <TableHead v-if="canAny(['tax.update', 'tax.delete'])" class="text-right w-[160px]">İşlemler</TableHead>
                                </TableRow>
                            </TableHeader>
                            <TableBody>
                                <TableRow v-for="tax in taxes" :key="tax.id" class="hover:bg-muted/50 transition-colors">
                                    ...
                                    <TableCell v-if="canAny(['tax.update', 'tax.delete'])" class="text-right">
                                        <div class="flex justify-end gap-2">
                                            <Button v-if="can('tax.update')" variant="outline" size="sm" class="h-8" @click="openEditSheet(tax)">
                                                <Pencil class="mr-2 h-3.5 w-3.5" />
                                                Düzenle
                                            </Button>
                                            <Button v-if="can('tax.delete')" variant="outline" size="sm" class="h-8 text-destructive border-destructive/30 hover:bg-destructive/5" @click="openDeleteDialog(tax)">
            ...
            <Sheet v-model:open="isSheetOpen">
                <SheetContent side="right" class="sm:max-w-[480px] p-0 flex flex-col h-full">
                    ...
                    <form id="taxForm" @submit.prevent="submitForm" class="flex-1 overflow-y-auto p-6 space-y-6">
                        <div class="space-y-2">
            ...
            <AlertDialog v-model:open="showDeleteDialog">
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            <strong>{{ deletingTax?.name }}</strong> kalıcı olarak silinecektir. Bu işlem geri alınamaz.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
