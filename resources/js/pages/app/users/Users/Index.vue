            ...
            <div class="space-y-6">
                ...
                <template v-else-if="users.data.length > 0">
                                ...
                                <TableRow v-for="user in users.data" :key="user.id"
                                    ...
                                    <TableCell>
                                        <div class="flex items-center gap-1.5">
                                            {{ user.email }}
                                            <CheckCircle2 v-if="user.email_verified_at"
                                    ...
                                    <TableCell v-if="can('user.update')" class="text-right">
                                        <Link
                                            :href="user.id === authUser?.id ? profileEdit().url : `${userRoute().url}/${user.id}/edit`">
                    ...
                    <div v-if="users.last_page > 1" class="flex items-center justify-between px-4 py-2">
                        <div class="text-sm text-muted-foreground">
                            Toplam <strong>{{ users.total }}</strong> kayıttan <strong>{{ users.from }}-{{ users.to
                                }}</strong> arası gösteriliyor.
                        </div>
                        <Pagination :total="users.total" :items-per-page="users.per_page"
                            :default-page="users.current_page" @update:page="handlePageChange">
                            <PaginationContent>
                ...
                <div v-else class="mx-2">
                    <EmptyState title="Kullanıcı Bulunamadı"
                        description="Sistemde kayıtlı veya aradığınız kritere uygun kullanıcı bulunmuyor." :icon="Users"
